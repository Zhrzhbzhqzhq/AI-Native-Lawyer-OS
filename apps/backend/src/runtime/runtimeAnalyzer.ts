...
export default function analyzeRuntimeState(snapshotFacts: any): RuntimeStateItem[] {
  ...
  if ((counts.evidence || 0) === 0) out.push({ code: 'NEEDS_EVIDENCE', value: true })

  // HAS_WEAK_EVIDENCE: number of weak evidence items (only include if >0)
  const weak = Number(ev.weak || 0)
  if (weak > 0) out.push({ code: 'HAS_WEAK_EVIDENCE', value: weak })

  // HAS_DRAFT_DOCUMENTS
  const draft = Number(docs.draft || 0)
  if (draft > 0) out.push({ code: 'HAS_DRAFT_DOCUMENTS', value: draft })

  // HAS_FINAL_DOCUMENTS
  const finalDocs = Number(docs.final || 0)
  if (finalDocs > 0) out.push({ code: 'HAS_FINAL_DOCUMENTS', value: finalDocs })

  // HAS_ARCHIVED_DOCUMENTS
  const archived = Number(docs.archived || 0)
  if (archived > 0) out.push({ code: 'HAS_ARCHIVED_DOCUMENTS', value: archived })
  ...
  if (recent > 0) {
    out.push({ code: 'RECENT_ACTIVITY', value: recent })
  } else {
    out.push({ code: 'NO_RECENT_ACTIVITY', value: true })
  }

  // NO_RESEARCH
  if ((counts.research || 0) === 0) out.push({ code: 'NO_RESEARCH', value: true })
