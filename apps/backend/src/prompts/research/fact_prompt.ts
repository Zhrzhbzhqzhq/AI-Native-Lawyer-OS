...
5) 最多返回 8 条核心事实，按重要性排序（最重要在前）；同一客观事实由多项证据共同支持时应合并表达，避免重复或按 Evidence 数量机械拆分。
6) 仅返回合法的 JSON 数组，不得包含解释、注释、Markdown 或额外字段；输出必须可以被机器解析为 JSON 并严格遵守上述对象字段（'title', 'description', 'category', 'evidence_titles'）。
7) Fact 只能陈述可观察、可核验的客观信息，尽可能写明时间、主体、行为、对象、结果和证据来源。
8) title 与 description 不得包含法律评价、责任结论、证明力判断或未经材料支持的推定，例如：构成违约、构成侵权、应承担责任、合同有效或无效、法律关系成立、应当认定、应予支持、足以证明、应推定、应视为。
9) 应将法律结论改写为客观事实。例如，不写“对方构成违约”，而写“对方在约定期限届满后仍未完成约定行为”；不写“质量异议不能成立”，而写“对方提出质量异议”。
10) 可以客观转述材料记载的约定或主张，但必须明确其来源。例如可写“合同文本载明逾期付款责任”，不得直接写“对方应承担逾期付款责任”。
11) 每条 description 不超过 180 个中文字符；在信息完整的前提下保持简洁，确保整个 JSON 数组完整闭合。
