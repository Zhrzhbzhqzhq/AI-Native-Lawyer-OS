...
export class FactUnderstandingArtifactStore {
  ...
  async createRun(matterId: string, runId = randomUUID()) {
    if (!/^[a-zA-Z0-9_-]+$/.test(runId)) throw new Error('fact_artifact_run_id_invalid')
    const directory = path.join(this.repositoryRoot, 'storage/context-engine-c0.4/fact-understanding', safeMatterKey(matterId), runId)
    await mkdir(directory, { recursive: true })
    const uri = (name: string) => path.relative(this.repositoryRoot, path.join(directory, name)).split(path.sep).join('/')
    return {
      runId,
      directory,
      inputUri: uri('fact-input.json'),
      rawResponseUri: uri('raw-ai-response.json'),
      resultUri: uri('fact-drafts.json'),
    }
  }

  saveInput(run: { directory: string }, value: unknown) { return this.writeJson(path.join(run.directory, 'fact-input.json'), value) }
  ...
  saveResult(run: { directory: string }, value: unknown) { return this.writeJson(path.join(run.directory, 'fact-drafts.json'), value) }
...
export default FactUnderstandingArtifactStore
