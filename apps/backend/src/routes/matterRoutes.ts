...
export async function matterRoutes(app: FastifyInstance) {
    ...
    try {
      ...
      try {
        ...
        if (svcAny && typeof svcAny.analyzeIssues === 'function') {
          ...
          return reply.code(200).send(Array.isArray(out) ? out : (out && out.issues ? out.issues : []))
        }
      } catch (e) {
        // continue to next fallback
      }

      // legacy: allow aiSuggestionService to provide analyzeIssues if present
      try {
        const svcAny2 = aiSuggestionService as any
        if (svcAny2 && typeof svcAny2.analyzeIssues === 'function') {
          const out = await svcAny2.analyzeIssues(matter_id)
          return reply.code(200).send(out)
        }
      } catch (e) {
        // fall back to mock
  ...
  // Laws CRUD
  app.post('/matters/:id/laws', async (request, reply) => {
    const { id } = request.params as any;
    const payload = request.body as any || {};
    const title = String(payload.title || '').trim();
    if (!title) return reply.code(400).send({ error: 'title required' });
    try {
      const created = await lawService.createLaw(id, { title, citation: String(payload.citation || ''), description: String(payload.description || ''), status: String(payload.status || 'draft'), issue_id: typeof payload.issue_id === 'string' ? String(payload.issue_id) : undefined });
      return reply.code(201).send(created);
    } catch (err: any) {
      return reply.code(500).send({ error: 'create_failed', detail: err?.message || String(err) });
    }
  ...
  app.get('/matters/:id/laws', async (request, reply) => {
    const { id } = request.params as any;
    try {
      const list = await lawService.listLaws(id);
      return reply.code(200).send(list);
    } catch (err: any) {
      return reply.code(500).send({ error: 'failed', detail: err?.message || String(err) });
    }
  ...
  app.patch('/matters/:matter_id/laws/:law_id', async (request, reply) => {
    const { law_id } = request.params as any;
    const payload = request.body as any || {};
    const patch: any = {};
    if (typeof payload.title === 'string') patch.title = String(payload.title);
    if (typeof payload.description === 'string') patch.description = String(payload.description);
    if (typeof payload.citation === 'string') patch.citation = String(payload.citation);
    if (typeof payload.status === 'string') patch.status = String(payload.status);
    if (typeof payload.issue_id === 'string') patch.issue_id = String(payload.issue_id);
    if (Object.keys(patch).length === 0) return reply.code(400).send({ error: 'nothing to update' });
    try {
      const updated = await lawService.updateLaw(law_id, patch);
      return reply.code(200).send(updated);
    } catch (err: any) {
      if (String(err.message) === 'Not found') return reply.code(404).send({ error: 'not_found' });
      return reply.code(500).send({ error: 'update_failed', detail: err?.message || String(err) });
  ...
  app.delete('/matters/:matter_id/laws/:law_id', async (request, reply) => {
    const { law_id } = request.params as any;
    try {
      await lawService.deleteLaw(law_id);
      return reply.code(204).send();
    } catch (err: any) {
      if (String(err.message) === 'Not found') return reply.code(404).send({ error: 'not_found' });
      return reply.code(500).send({ error: 'delete_failed', detail: err?.message || String(err) });
