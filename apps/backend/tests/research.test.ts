...
beforeAll(async () => {
  ...
  await prisma.knowledge.deleteMany({ where: { matter_id: testMatterId } }).catch(() => { })
...
describe('Research API', () => {
  it('creates, lists, and deletes research records for a matter', async () => {
    ...
    expect(created.knowledge_id).toBeDefined()
    ...
    const found = list.find((t: any) => t.knowledge_id === created.knowledge_id)
    expect(found).toBeTruthy()

    // delete
    const delRes = await app.inject({ method: 'DELETE', url: `/matters/${matterId}/research/${created.knowledge_id}` })
    ...
    const found2 = list2.find((t: any) => t.knowledge_id === created.knowledge_id)
