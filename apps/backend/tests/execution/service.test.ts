...
describe('ExecutionService transitions', () => {
  it('start: PENDING -> RUNNING', async () => {
    ...
    const res = await svc.start(TEST_MATTER, qid)
  ...
  it('pause: RUNNING -> PENDING', async () => {
    ...
    const res = await svc.pause(TEST_MATTER, qid)
  ...
  it('complete: RUNNING -> DONE', async () => {
    ...
    const res = await svc.complete(TEST_MATTER, qid)
  ...
  it('non-existent queue_id should throw', async () => {
    await expect(svc.start(TEST_MATTER, 'no-such-queue')).rejects.toThrow()
    await expect(svc.pause(TEST_MATTER, 'no-such-queue')).rejects.toThrow()
    await expect(svc.complete(TEST_MATTER, 'no-such-queue')).rejects.toThrow()
  })
})
