          ...
          <div id="doc-workflow" style={{ background: lawdesk.cardBg, padding: 12, borderRadius: lawdesk.radius, border: `1px solid ${lawdesk.border}` }}>
            <div style={{ fontWeight: 800, color: lawdesk.text }}>文书类型</div>
              ...
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {recommended.map((r) => (
                  <div key={r.id} style={{ padding: 10, background: '#fff', borderRadius: 8, border: `1px solid ${lawdesk.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontWeight: 700 }}>{'★'.repeat(r.stars) + '☆'.repeat(5 - r.stars)} {r.title}</div>
                    </div>
                    <div>
